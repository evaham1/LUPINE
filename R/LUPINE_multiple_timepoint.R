...
#' If used, refression models will be run on log-transformed data with library size as an offset.
...
LUPINE_multiple_timepoint <- function(data,
  ...
  compute_partial_corr <- function(i) {
    ...
    u1 <- scale(data_final_timepoint, center = TRUE, scale = TRUE) %*% loading_tmp # Dimension reduced data

    # Fit linear models for both variables - may account for library size / log normalise
    r_i <- if (is.null(lib_size)) {
      lm(data_final_timepoint[, var1[i]] ~ u1)
    } else {
      lm(log(data_final_timepoint[, var1[i]] + 1) ~ u1, offset = log(lib_size[, dim(data)[3]]))
    }
    r_j <- if (is.null(lib_size)) {
      lm(data_final_timepoint[, var2[i]] ~ u1)
    } else {
      lm(log(data_final_timepoint[, var2[i]] + 1) ~ u1, offset = log(lib_size[, dim(data)[3]]))
    }
